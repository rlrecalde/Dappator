...
namespace Dappator.MySql.Test
{
    public class Functional : TestBase
    {
        ...
        [Test]
        public void Insert_With_Object_Bulk()
        {
            #region Arrange
        ...
        public void Select_With_Cast()
        {
            ...
            Model.DataTypeNullable dataType = dataContext
                ...
                .Cast<Model.DataTypeNullable>(x => x.Float, "DECIMAL")
                .Cast<Model.DataTypeNullable>(x => x.Double, "DECIMAL")
                .QueryFirstOrDefault<Model.DataTypeNullable>();
        ...
        [Test]
        public void Updates_In_Transactions_IsolationLevel_Default()
        {
            #region Arrange
            ...
            string nickToUpdate = $"{user.Nick}_2";
            string passwordToUpdate = $"{user.Password}_2";

            string nick2ToUpdate = $"{user.Nick}_3";
            string password2ToUpdate = $"{user.Password}_3";

            #endregion

            #region Act

            Model.User userRead = null;
            var tasks = new List<Task>();
            bool task1HasInserted = false;

            // Here we have two tasks. Both of them fire almost at the same time.
            // task1 tries to update a record.
            // task2 tries to update the same record.
            // Stage 1:
            //   task1 updates the record and raises a flag indicating this action, but doesn't commit or rollback yet.
            // Stage 2:
            //   task2, after this flag was raised, reads the record and updates it with different values but 
            // can't commit it until the previous transaction is complete. So, Execute() statement stays still.
            // Stage 3:
            //   After a period of 2 seconds, task1 rollbacks its action.
            // Stage 4:
            //   After that transaction completion, task2 is now able to commit its action.

            var task2 = Task.Run(() =>
            {
                while (!task1HasInserted)
                    Task.Delay(100).Wait();
                ...
                userRead = dataContext
                    .Select<Model.User>(x => new { x.Id, x.Nick, x.Password })
                    .Where<Model.User>(x => x.Id, Common.Operators.EqualTo, user.Id)
                    .OrderBy<Model.User>(x => x.Id, Common.Orders.Ascending)
                    .LimitOffset(1)
                    .QueryFirst<Model.User>();
            ...
            Assert.That(userRead.Nick, Is.EqualTo(user.Nick));

            #endregion
        }

        [Test]
        public async Task Updates_In_Transactions_IsolationLevel_Default_Async()
        {
            #region Arrange
            ...
            var task2 = Task.Run(async () =>
            {
                ...
                userRead = await dataContext
                    .Select<Model.User>(x => new { x.Id, x.Nick, x.Password })
                    .Where<Model.User>(x => x.Id, Common.Operators.EqualTo, user.Id)
                    .OrderBy<Model.User>(x => x.Id, Common.Orders.Ascending)
                    .LimitOffset(1)
                    .QueryFirstAsync<Model.User>();
        ...
        [Test]
        public void Updates_In_Transactions_IsolationLevel_ReadUncommitted()
        {
            #region Arrange
            ...
            var task2 = Task.Run(() =>
            {
                ...
                dataContext.ExecuteInTransaction = true;
                dataContext.TransactionIsolationLevel = System.Data.IsolationLevel.ReadUncommitted;
            ...
            Assert.That(userRead.Nick, Is.EqualTo(nickToUpdate));

            #endregion
        }

        [Test]
        public async Task Updates_In_Transactions_IsolationLevel_ReadUncommitted_Async()
        {
            #region Arrange
        ...
        public void SpInsertUserAndGetId()
        {
            ...
            int userId = (int)dataContext
                .StoredProcedure<Model.SpInsertUserAndGetId>(x => new { x.Nick, x.Password }, user.Nick, user.Password)
                .ExecuteAndRead<long>();
        ...
        public void SpGetUserById()
        {
            ...
            Model.User user = dataContext
                .StoredProcedure<Model.SpGetUserById>(x => new { x.Id }, userToInsert.Id)
                .ExecuteAndRead<Model.User>();
        ...
        public async Task SpGetUserById_Async()
        {
            ...
            Model.User user = await dataContext
                .StoredProcedure<Model.SpGetUserById>(x => new { x.Id }, userToInsert.Id)
                .ExecuteAndReadAsync<Model.User>();
        ...
        public void SpGetUsers()
        {
            ...
            IEnumerable<Model.User> users = dataContext
                .StoredProcedure<Model.SpGetUsers>()
                .ExecuteAndRead<IEnumerable<Model.User>>();
        ...
        public async Task SpGetUsers_Async()
        {
            ...
            IEnumerable<Model.User> users = await dataContext
                .StoredProcedure<Model.SpGetUsers>()
                .ExecuteAndReadAsync<IEnumerable<Model.User>>();
