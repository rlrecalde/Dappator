# Dappator - Strongly Typed Query Builder + Data Context with Dapper for Oracle
...
namespace Whatever
{
    public class DataAccess
    {
        ...
        {
            using var dataContext = new Dappator.Oracle.DataContext("connectionString");
...
When DataContext is instantiated, it creates an OracleConnection object with the supplied connectionstring.
...
Finally, when DataContext object is disposed, it closes and disposes the OracleConnection object.
...
2. An OracleConnection manager, where you don't have to create, open, close and dispose a connection manually anymore.
...
It is responsible of managing the OracleConnection object.

It has the following properties and methods:
- ConnectionString (readonly): Gets the connectionstring.
- DbConnection (readonly): Gets the OracleConnection object.
...
- TableFunction(): Starts the construction of the "SELECT * FROM TABLE([function name] ([values]))" statement.
- ScalarFunction(): Starts the construction of the "SELECT [function name] ([values]) FROM DUAL" statement.
- SetQuery(): Just sends a string query to Dapper, in case you don't want to build it with the available methods.
- Close() / CloseAsync(): Closes the OracleConnection object.
...
namespace Whatever
{
    public static class IoC
    {
        public static void ConfigureServices(IServiceCollection services, string connectionString)
        {
            ...
            {
                return new Dappator.Oracle.DataContext(connectionString);
            });
        }
    }
}
...
```
Then, declare your own DataContext class which derives from Dappator.Oracle.DataContext and implements your interface:
``` csharp
namespace Whatever.Model
{
    public class MyDatabaseContext : Dappator.Oracle.DataContext, IMyDatabaseContext
    {
...
RETURNING CAST(Id AS NUMBER) INTO :xxx;
END;
...
long myId = dataContext
    ...
    .ExecuteScalar();
```
Since Insert() method always creates the "RETURNING CAST(Id AS NUMBER)" string after the "INSERT INTO..." statement, introducing the Returning() method makes it replace/change the substring "Id" with whatever string was set on Description attribute for the property.
...
SELECT
   ...
   CAST("User".Payment AS NUMBER) AS Payment 
FROM "User"
```
For `property` parameter, a lambda expression with a property of T must be supplied.  
...
SELECT
   ...
   u2.Id,
   u2.Nick,
   CAST(u2.Salary AS NUMBER) AS Salary,
...
IEnumerable<SomeClass> some = await dataContext
    ...
    .Cast<User>(x => x.Salary, "NUMBER", alias: "u2")
...
2. Once you are sure that everything is working fine, replace Dapper and everything related to the OracleConnection object with Dappator (DataContext) and begin using Dappator from now on.
