# Dappator - Strongly Typed Query Builder + Data Context with Dapper for Sqlite
...
namespace Whatever
{
    public class DataAccess
    {
        ...
        {
            using var dataContext = new Dappator.Sqlite.DataContext("connectionString");
...
When DataContext is instantiated, it creates a SqliteConnection object with the supplied connectionstring.
...
Finally, when DataContext object is disposed, it closes and disposes the SqliteConnection object.
...
2. A SqliteConnection manager, where you don't have to create, open, close and dispose a connection manually anymore.
...
It is responsible of managing the SqliteConnection object.

It has the following properties and methods:
- ConnectionString (readonly): Gets the connectionstring.
- DbConnection (readonly): Gets the SqliteConnection object.
...
- StoredProcedure(): Reserved for other providers. Sqlite does not support stored procedures.
- TableFunction(): Reserved for other providers. Sqlite does not support table-valued functions.
- ScalarFunction(): Reserved for other providers. Sqlite does not support scalar-valued functions.
- SetQuery(): Just sends a string query to Dapper, in case you don't want to build it with the available methods.
- Close() / CloseAsync(): Closes the SqliteConnection object.
...
namespace Whatever
{
    public static class IoC
    {
        public static void ConfigureServices(IServiceCollection services, string connectionString)
        {
            ...
            {
                return new Dappator.Sqlite.DataContext(connectionString);
            });
        }
    }
}
...
Sometimes, you want to do functional tests for another provider and using Sqlite as an in-memory database (which needs two different connections/providers for the same injected interface).  
...
```
Then, declare your own DataContext class which derives from Dappator.Sqlite.DataContext and implements your interface:
``` csharp
namespace Whatever.Model
{
    public class MyDatabaseContext : Dappator.Sqlite.DataContext, IMyDatabaseContext
    {
...
Given that Sqlite has the ability of just setting LIMIT with no OFFSET when it's not needed, in case you do the following sentence:
``` csharp
IEnumerable<User> users = await dataContext
    .Select<User>(x => new { x.Id, x.Nick, x.Password })
    .OrderBy<User>(x => x.Id, Dappator.Common.Orders.Ascending)
...
2. Once you are sure that everything is working fine, replace Dapper and everything related to the SqliteConnection object with Dappator (DataContext) and begin using Dappator from now on.
...
An another validation is that every class used as T on every method must have a Description attribute for the class and Description attributes for its properties.  
